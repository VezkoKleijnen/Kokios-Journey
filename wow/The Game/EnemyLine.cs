...
using GXPEngine;
internal class EnemyLine : LineSegment
{
    float xOffS;
    float yOffS;
    float xOffE;
    float yOffE;
    public int health;
    public Pushable enemy;
    int stayColor;
    float myRotation;
    public EnemyLine(Pushable _enemy, float _xOffS, float _yOffS, float _xOffE, float _yOffE) : base(_xOffS-_enemy.position.x, _yOffS - _enemy.position.y, _xOffE - _enemy.position.x, _yOffE - _enemy.position.y, 0xffffffff, 2)
    {
        health = 6;
        xOffS = _xOffS;
        yOffS = _yOffS;
